...
class ProjectStorage {
    ...
    async saveLocally(data, image) {
        ...
        if (!this.data.Projects[c]) {
            this.initProjectStructure(c);
        }

        try {
            await this.saveToStorage(c, data); // Save data to OPFS or IndexedDB based on support
        } catch (error) {
    ...
    async getCurrentProjectData() {
        ...
        const c = this.data.CurrentProject;
        try {
            return await this.loadFromStorage(c); // Load data from OPFS or IndexedDB based on support
        } catch (error) {
    ...
    // Add published data to a project
    ...
    async deleteProject(id) {
        ...
        this.save();
        await this.deleteFromStorage(id); // Delete from OPFS or IndexedDB based on support
    ...
    // Check if a project is reported
    isReported(id) {
        return this.data.ReportedProjects[id] === true;
    }

    // Report/unreport a project
    report(id, report) {
    ...
    async migrateToNewStorage() {
        for (const [id, project] of Object.entries(this.data.Projects)) {
            if (project.ProjectData) {
                await this.saveToStorage(id, project.ProjectData);
    ...
    // New methods for OPFS integration

    async initOPFS() {
        if (!this.originPrivateFS) {
            this.originPrivateFS = await navigator.storage.getDirectory();
        }
    }

    async saveToStorage(projectId, data) {
        if (this.isOPFSSupported()) {
            await this.saveToOPFS(projectId, data);
        } else {
            await this.LocalStorage.setItem(`${this.LocalStorageKey}_${projectId}`, JSON.stringify(data));
        }
    }

    async loadFromStorage(projectId) {
        if (this.isOPFSSupported()) {
            return await this.loadFromOPFS(projectId);
        } else {
            const projectData = await this.LocalStorage.getItem(`${this.LocalStorageKey}_${projectId}`);
            return projectData ? JSON.parse(projectData) : null;
        }
    }

    async deleteFromStorage(projectId) {
        if (this.isOPFSSupported()) {
            await this.deleteFromOPFS(projectId);
        } else {
            await this.LocalStorage.removeItem(`${this.LocalStorageKey}_${projectId}`);
        }
    }
    ...
    async saveToOPFS(projectId, data) {
        await this.initOPFS();
        const fileHandle = await this.originPrivateFS.getFileHandle(`${projectId}.mbt`, { create: true });
        const writable = await fileHandle.createWritable();
        await writable.write(new TextEncoder().encode(data));
        await writable.close();
    }

    async loadFromOPFS(projectId) {
        await this.initOPFS();
        const fileHandle = await this.originPrivateFS.getFileHandle(`${projectId}.mbt`);
        const file = await fileHandle.getFile();
        return await file.text();
    }

    async deleteFromOPFS(projectId) {
        try {
            await this.initOPFS();
            await this.originPrivateFS.removeEntry(`${projectId}.mbt`);
        } catch (error) {
